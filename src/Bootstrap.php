...
use RekticWp\Common\Traits\Requester;
use RekticWp\Common\Utils\Errors;
use RekticWp\Config\Classes;
use RekticWp\Config\I18n;
use RekticWp\Config\Requirements;
use Throwable;
...
final class Bootstrap extends Base{
	...
	public function loadClasses($classes){
		...
		foreach($classes as $class){
			...
			else if(isset($class['on_request']) && !$this->request($class['on_request'])){
				continue;
			}
	...
	public function getClasses(string $namespace): array{
		...
		// Errors::writeLog(
		//    [
		//        'title'   => __( 'Rektic WP classes are not being loaded by Composer\'s Autoloader' ),
	...
	public function extractClasses($file_data, $classes = []): array{
		for($index = 0; isset($file_data['tokens'][$index]); $index++){
			...
			if(T_CLASS === $file_data['tokens'][$index][0] && T_WHITESPACE === $file_data['tokens'][$index + 1][0] &&
	...
	public function initClasses(){
		$this->class_list = apply_filters('rektic_wp_initialized_classes', $this->class_list);
		foreach($this->class_list as $class){
			try{
				...
				do_action('rektic_wp_class_initialize_failed', $err, $class);
				Errors::wpDie(sprintf(__('Could not load class "%s". The "init" method is probably missing or try a `composer dumpautoload -o` to refresh the autoloader.',
										 'rektic-wp'), $class), __('Plugin initialize failed', 'rektic-wp'), __FILE__, $err);
			}
		}
	}
	...
	public function debugger(){
		if($this->bootstrap['debug'] === true){
			$this->bootstrap['execution_time'] = 'Total execution time in seconds: ' .
												 (microtime(true) - $this->bootstrap['execution_time']['start']);
