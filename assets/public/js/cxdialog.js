...
(
	function(global, factory){
		...
		define(factory) :
		(
			global = typeof globalThis !== "undefined" ? globalThis : global || self, global.cxDialog = factory()
		);
	}
)(this, (
	function(){
		...
		theTool.setContent = function(){
			...
			for(let i = 0,
					l = self.settings.buttons.length; i < l; i ++){
				self.cacheBtnFun.push({
			...
			if(self.cacheDom){
				if(self.dom.box.querySelector(".cxdialog_title")){
					self.dom.box.querySelector(".cxdialog_title")
						.insertAdjacentElement("afterend", self.cacheDom.el);
				}
				else{
		...
		theTool.backDom = function(){
			...
			if(self.dom.box.querySelector(".cxdialog_title")){
				self.dom.box.querySelector(".cxdialog_title")
					.insertAdjacentElement("afterend", self.dom.holder);
			}
			else{
